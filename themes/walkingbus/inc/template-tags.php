 ...
 */
  ...
  * @return bool
  */
 function wsb_categorized_blog() {
 	if ( false === ( $all_the_cool_cats = get_transient( 'wsb_categories' ) ) ) {
 		...
 		set_transient( 'wsb_categories', $all_the_cool_cats );
 	}

 	if ( $all_the_cool_cats > 1 ) {
 		// This blog has more than 1 category so wsb_categorized_blog should return true.
 		return true;
 	} else {
 		// This blog has only 1 category so wsb_categorized_blog should return false.
 		return false;
 	}
 }

 /**
  * Flush out the transients used in wsb_categorized_blog.
  */
 function wsb_category_transient_flusher() {
 	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
 		return;
 	}
 	// Like, beat it. Dig?
 	delete_transient( 'wsb_categories' );
 }
 add_action( 'edit_category', 'wsb_category_transient_flusher' );
 add_action( 'save_post',     'wsb_category_transient_flusher' );
  ...
  */
 function wsb_comment_list( $comment, $args, $depth ) {
  ...
  */
 function wsb_numbered_pagination() {
 	global $wp_query;
 	$big = 999999999;
